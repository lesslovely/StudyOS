...
## 待确认问题（进入 Phase 1 时由开发者拍板）

1. P1 是否附带 D-010 所述的 JavaScript 最小示例知识集，还是从空白领域开始？
2. P1 学习状态枚举建议为 `未学习 / 学习中 / 复习中 / 已掌握`，是否接受？
3. P1 视图数量：建议“知识总览（含薄弱点、建议、待办、最近记录）/ 知识详情 / 任务与记录”三个视图，是否接受？
4. 掌握度自评控件形态（滑块 1–100 还是档位：生疏 / 入门 / 熟悉 / 掌握），进入 P1 后按学习理解选择。
5. 当前暂存区的空脚手架（`src/index.html`、`.gitkeep`）在 P1 的 M0 提交为基线，是否同意？
